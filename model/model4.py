...
class Model(nn.Module):
    def __init__(self, laplace, train_shape, pre_step, device):
        ...
        self.xhr_GRU = nn.ModuleList(
            [CustomGRUCell(3 * self.head_out * len(self.head_t_ahead), self.head_out * len(self.head_t_ahead), train_shape[1]) for i in range(11)]
        )
    ...
    def forward(self, x, T):
        ...
        att_t, att_g= [], []
        for head_t, head_l in zip(self.head_t_ahead, self.head_g_ahead):
        ...
        for i in range(1, att_g_out.shape[1]):
            tensor1 = att_t_out[:, i, :, :]
            tensor3 = att_g_out[:, i, :, :]
            tensor4 = att_t_out[:, i+1, :, :]
            st_cat = torch.cat((tensor1, tensor3, tensor4), dim=-1)
