from traffic_prediction1.model.model import Model
...
def test_(args):
    ...
    model = Model(laplace, train_data.shape, args.prediction_time, device,std_mean).to(device)
    PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))  # 当前文件目录
    PARENT_DIR = os.path.dirname(PROJECT_ROOT)
    print(PARENT_DIR)
    print(PARENT_DIR, f"data/weights_file/{args.pems}_{args.prediction_time}.pth")
    model.load_state_dict(
        torch.load(os.path.join(PARENT_DIR
                                , f"data/weights_file/{args.pems}_{args.prediction_time}.pth"),
                   map_location=device))
    print(PARENT_DIR, f"data/weights_file/{args.pems}_{args.prediction_time}.pth")

    test_loader = DataLoader(TrafficDataset(test_data, std_mean, args.prediction_time),
