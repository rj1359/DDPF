...
class Model(nn.Module):
    def __init__(self, laplace, train_shape, pre_step, device, std_mean):
        ...
        self.std = std_mean[0][0]
        self.mean = std_mean[0][1]
        ...
        self.last_GRU = nn.ModuleList(
            [CustomGRUCell(2 * self.head_out * len(self.head_t_ahead), self.head_out * len(self.head_t_ahead), train_shape[1]) for i in range(pre_step)]
        )
        self.out = nn.Linear(self.head_out * len(self.head_t_ahead), 1)
        print("完全体模型初始化完成")

    def forward(self, x, T):
        fx_1 = (x[:, :, :, 0:1] - self.mean) / self.std
        ...
        for i in range(1, att_g_out.shape[1]):
            ...
            tensor2 = att_s_out[:, i, :, :]
            tensor3 = att_g_out[:, i, :, :]
            tensor4 = att_stt_out[:, i+1, :, :]
            st_cat = torch.cat((tensor1, tensor2, tensor3, tensor4), dim=-1)
