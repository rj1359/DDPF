...
class Model(nn.Module):
    def __init__(self, laplace, train_shape, pre_step, device):
        ...
        self.conv = nn.Conv2d(in_channels=train_shape[-1], out_channels=self.head_out * len(self.head_s_ahead),
                              kernel_size=[1, int(pre_step / 5) + 11], padding=0)
        self.conv1 = nn.Conv2d(in_channels=train_shape[-1], out_channels=self.head_out * len(self.head_s_ahead),
                              kernel_size=[1, int(pre_step / 5) + 12], padding=0)
        self.pre_laplace_linre = nn.Linear(in_features=1, out_features=self.head_out * len(self.head_s_ahead))
        self.pre_x_laplace_linre = nn.Linear(in_features=1, out_features=self.head_out * len(self.head_s_ahead))
        ...
        self.residual_proj = nn.Linear(train_shape[-1], self.head_out * len(self.head_s_ahead))

        self.xhr_GRU = nn.ModuleList(
            [CustomGRUCell(3 * self.head_out * len(self.head_s_ahead), self.head_out * len(self.head_s_ahead), train_shape[1]) for i in range(11)]
        )

        self.last_GRU = nn.ModuleList(
            [CustomGRUCell(2 * self.head_out * len(self.head_s_ahead), self.head_out * len(self.head_s_ahead), train_shape[1]) for i in range(pre_step)]
        )
        self.out = nn.Linear(self.head_out * len(self.head_s_ahead), 1)
        print("完全体模型初始化完成")

    def forward(self, x, T):
        fx_1 = x[:, :, :, 0:1]
        fx_laplace = torch.matmul(self.laplace, fx_1)  # 空间异质性
        fxt_1 = x[:, 1:, :, :]
        fxt_1[:, :, :, 0:1] = fx_1[:, 1:, :, :] - fx_laplace[:, :-1, :, :]

        fx_1 = torch.cat((fx_1, x[:, :, :, 1:]), dim=-1)  # 加入时间信息
        time_pre = torch.cat((T.new_zeros(T.shape[0], T.shape[1], T.shape[2], 1), T), dim=-1)
        time_pre = torch.cat((fxt_1, time_pre), dim=1)
        time_pre = torch.cat((time_pre, time_pre[:, :self.pre_step - 1, :, :]), dim=1)
        pre_laplace = self.conv(time_pre.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
        ...
        att_s, att_st = [], []
        for head_s, head_st in zip(self.head_s_ahead, self.head_st_ahead):
            att_s.append(head_s(fxt_1.permute(0, 2, 1, 3)).permute(0, 2, 1, 3))
            att_st.append(head_st(fx_1.permute(0, 2, 1, 3)).permute(0, 2, 1, 3))
        att_s_cat = torch.cat(att_s, dim=-1)
        att_st_cat = torch.cat(att_st, dim=-1)
        att_s_out = self.out_proj_s(att_s_cat)
        att_st_out = self.out_proj_st(att_st_cat)

        ht = att_st_out.new_zeros(att_st_out[:, 0, :, :].shape)
        for i in range(1, att_s_out.shape[1]):
            tensor1 = att_st_out[:, i, :, :]
            tensor2 = att_s_out[:, i, :, :]
            tensor4 = att_st_out[:, i+1, :, :]
            st_cat = torch.cat((tensor1, tensor2, tensor4), dim=-1)
            ht = self.xhr_GRU[i](st_cat, ht)

        out = pre_laplace_x.new_zeros(pre_laplace_x.shape)
        for i in range(T.shape[1]):
            tensor1 = pre_laplace[:, i, :, :]
            tensor2 = pre_laplace_x[:, i, :, :]
            st_cat = torch.cat((tensor1, tensor2), dim=-1)
            ht = self.last_GRU[i](st_cat, ht)
            out[:, i, :, :] = ht
        out = self.out(out)
        return out